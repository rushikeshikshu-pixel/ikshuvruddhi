...
Cohort Analysis and Stratified Diagnostic Reporter
"""
...
def analyze_cohorts(csv_path=CSV_PATH):
    ...
    print(f" High-NDVI / Strong Vegetation Stratum (NDVI >= 0.55, N={len(high_ndvi)}):")
    print(f"   -> Mean Parcel-Masked NDVI                 : {high_ndvi['mean_field_ndvi'].mean():.3f}")
    print(f"   -> Mean Parcel Cane Occupancy              : {high_ndvi['parcel_cane_occupancy_pct'].mean():.2f}%")
    ...
    print(f" Intermediate / Stressed / Mixed Stratum (0.35 <= NDVI < 0.55, N={len(mid_ndvi)}):")
    print(f"   -> Mean Parcel-Masked NDVI                 : {mid_ndvi['mean_field_ndvi'].mean():.3f}")
    print(f"   -> Mean Parcel Cane Occupancy              : {mid_ndvi['parcel_cane_occupancy_pct'].mean():.2f}%")
    print(f"   -> Mean Strict Parcel IoU                  : {mid_ndvi['strict_parcel_iou_pct'].mean():.2f}%")
    print("------------------------------------------------------------------")
    
    low_ndvi = df[df['mean_field_ndvi'] < 0.35]
    print(f" No Standing Vegetation / Fallow Stratum (NDVI < 0.35, N={len(low_ndvi)}):")
